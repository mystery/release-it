...
const { GitHub: GitHubActionsClient } = require('@actions/github');
...
// Look for the GitHub action id in the environment as an indicator that we're
// being run as part of a GitHub action. Ref: https://bit.ly/2vsyRzu
const IS_GITHUB_ACTION = !!process.env.GITHUB_ACTION;
const RETRY_CODES = [408, 413, 429, 500, 502, 503, 504, 521, 522, 524];

const GitHubClientConstructor = IS_GITHUB_ACTION ? GitHubActionsClient : Octokit;
...
class GitHub extends Release {
  ...
  get client() {
    ...
    const client = new GitHubClientConstructor(options);
