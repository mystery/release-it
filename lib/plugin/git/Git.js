...
class Git extends GitBase {
  ...
  commit({ message = this.options.commitMessage, args = this.options.commitArgs } = {}) {
    const msg = format(message, this.getContext());
    const ar = args ? (typeof args === 'string' ? args.split(' ') : args) : [];
    return this.exec(['git', 'commit', '--message', msg, ...ar]).then(
  ...
  tag({ name = this.options.tagName, annotation = this.options.tagAnnotation, args = this.options.tagArgs } = {}) {
    const message = format(annotation, this.getContext());
    const n = format(name, this.getContext());
    const ar = args ? (typeof args === 'string' ? args.split(' ') : args) : [];
    return this.exec(['git', 'tag', '--annotate', '--message', message, ...ar, n])
  ...
  async push({ args = this.options.pushArgs } = {}) {
    ...
    const ar = args ? (typeof args === 'string' ? args : args.join(' ')) : [];
    let upstream = '';
    ...
    return this.exec(`git push ${ar} ${upstream}`);
  }
